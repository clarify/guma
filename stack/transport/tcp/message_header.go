...
//
// This field is only meaningful for MessageType of ‘MSG’ This field is always
// ‘F’ for other MessageTypes.
const (
	ChunkTypeIntermediate ChunkType = 'C'
	ChunkTypeFinal        ChunkType = 'F'
	ChunkTypeFinalAborted ChunkType = 'A'
	MessageHeaderByteSize int       = 8
)
...
func (m MessageHeader) String() string {
	return fmt.Sprintf(`
		...
		ChunkType:	%c
		Size:		%d
		---- /MessageHeader ----`, m.Type, m.ChunkType, m.Size)
}

// UnmarshalBinary decodes an OPC UA MessageHeader from byte array data into
// MessageHeader struct m.
...
// BitLength returns the total bit size of the MessageHeader It implements the
